...
public class Reference extends Analyser {
    @Override
    ...
    public ClassNode matchClassNode(List<ClassNode> classes) {
        for (ClassNode classNode : classes) {
            if (!classNode.superName.equals(getClassAnalyser("EntityNode").getNode().name) || !Modifier.isAbstract(classNode.access)) {
                continue;
            }
            ...
            for (MethodNode methodNode : classNode.methods) {
                ...
                InstructionSearcher instructionSearcher = new InstructionSearcher(methodNode.instructions, 0, ALOAD, ILOAD, PUTFIELD);
